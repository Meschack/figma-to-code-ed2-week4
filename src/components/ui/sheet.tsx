  ...
  'fixed z-50 gap-4 bg-white p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out',
  {
    variants: {
      side: {
        ...
        left: 'inset-y-0 left-0 h-full w-[calc(100vw-40px)] border-r data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left sm:max-w-sm',
        right:
          'inset-y-0 right-0 h-full w-[calc(100vw-40px)] border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-sm'
      }
    },
...
const SheetHeader = ({
  ...
  <div
    className={cn(
      'flex h-16 shrink-0 flex-col justify-center space-y-2 border-b px-4',
      className
    )}
    {...props}
  >
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn(
      'text-lg font-semibold text-doctrin-neutral-primary',
      className
    )}
    {...props}
  />
